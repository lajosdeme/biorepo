...
type NewDesign struct {
	Sequence    types.Sequence
	Parent      CommitId // ZeroCommitId for root commits
...
func New(s store.Store, i Index) *Client {
	if s == nil {
...
func (c *Client) PublishDesign(ctx context.Context, d NewDesign) (CommitId, error) {
	if err := d.Sequence.Validate(); err != nil {
		return CommitId{}, fmt.Errorf("publishDesign: invalid sequence: %w", err)
	}

	// 1. Encode as FASTA and upload to Filecoin.
	var buf bytes.Buffer
	if err := d.Sequence.EncodeFASTA(&buf); err != nil {
		return CommitId{}, fmt.Errorf("publishDesign: encoding FASTA: %w", err)
	}
	cid, err := c.store.Put(ctx, buf.Bytes())
	if err != nil {
		return CommitId{}, fmt.Errorf("publishDesign: storing sequence: %w", err)
	}
	...
	commitId, err := c.index.Commit(ctx, CommitRequest{
		CID:         cid,
		Parent:      d.Parent,
		Author:      d.Author,
	...
	if err != nil {
		return CommitId{}, fmt.Errorf("publishDesign: committing to index: %w", err)
	}
...
func (c *Client) ForkDesign(ctx context.Context, parentId CommitId, d NewDesign) (CommitId, error) {
	if _, err := c.index.GetCommit(ctx, parentId); err != nil {
		return CommitId{}, fmt.Errorf("forkDesign: parent not found: %w", err)
	}
	d.Parent = parentId
...
func (c *Client) GetDesign(ctx context.Context, id CommitId) (Design, error) {
...
// of length 1.
func (c *Client) GetLineage(ctx context.Context, id CommitId) ([]Design, error) {
	...
	for {
		...
		if CommitId(commit.Parent).IsZero() {
			break
		}
...
func (c *Client) GetChildren(ctx context.Context, id CommitId) ([]Design, error) {
...
func (c *Client) resolveIds(ctx context.Context, ids []CommitId, caller string) ([]Design, error) {
